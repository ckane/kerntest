...
use acpi::{hpet::HpetInfo, AcpiError};
...
impl Driver for HpetDriver {
        ...
        Self: Sized,
    {
        if let Ok(hpet_info) = bus.get("hpet_info")?.downcast::<HpetInfo>() {
            ...
            if let Ok(hpet_remap) = unsafe {
                PhysMapper::map_phys::<u64>(hpet_base_addr, 0x1000, PatTypes::Uncacheable)
            } {
                let hpet_registers =
                    unsafe { core::slice::from_raw_parts(hpet_remap.get_vptr(), 0x1000 >> 3) };
                let hpet_gen = hpet_registers[0];
                let hpet_timer_count = ((hpet_gen & 0xf00) >> 8) + 1;
                info!(
                    "HPET @ {:#018x}->{:#018x}: Vendor: {:#06x}, Count: {}",
                    hpet_base_addr,
                    hpet_remap.get_vptr() as usize,
                    (hpet_gen & 0x0ffff0000) >> 16,
                    hpet_timer_count
                );
                ...
                Err(DriverError::Initialization {
                    reason: String::from("Failed mapping HPET"),
                })?
            }
        } else {
            Err(DriverError::Initialization {
