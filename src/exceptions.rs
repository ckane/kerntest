use crate::cpu::{preempt_thread, save_context};
...
extern "x86-interrupt" fn div0_trap(frame: InterruptStack) {
    panic!("Division by zero or overflow @ {:?}", frame);
}

extern "x86-interrupt" fn dbg_trap(frame: InterruptStack) {
    panic!("Debug trap @ {:?}", frame);
}

extern "x86-interrupt" fn nmi_int(frame: InterruptStack) {
    error!("NMI @ {:?}", frame);
}

extern "x86-interrupt" fn bp_trap(frame: InterruptStack) {
    error!("Breakpoint @ {:?}", frame);
    // This "fault" is recoverable
}

extern "x86-interrupt" fn of_trap(frame: InterruptStack) {
    error!("Overflow @ {:?}", frame);
    // This "fault" is recoverable
}

extern "x86-interrupt" fn bre_panic(frame: InterruptStack) {
    panic!("Bound Range Exceeded @ {:?}", frame)
}

extern "x86-interrupt" fn invop_panic(frame: InterruptStack) {
    panic!("Invalid opcode @ {:?}", frame)
}

extern "x86-interrupt" fn dna_panic(frame: InterruptStack) {
    panic!("Device not available @ {:?}", frame)
}

extern "x86-interrupt" fn df_panic(frame: InterruptStack, _error_code: u64) {
    panic!("Double fault @ {:?}", frame)
}

extern "x86-interrupt" fn cso_panic(frame: InterruptStack) {
    panic!(
        "Coprocessor segment overrun (SHOULD NOT HAPPEN) @ {:?}",
        frame
    )
}

extern "x86-interrupt" fn tss_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "Invalid TSS (IST on x64), error: {:#018x} @ {:?}",
        error_code, frame
    )
}

extern "x86-interrupt" fn segpres_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "Segment not Present, error: {:#018x} @ {:?}",
        error_code, frame
    )
}

extern "x86-interrupt" fn stkseg_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "Stack Segment Fault, error: {:#018x} @ {:?}",
        error_code, frame
    )
}

#[naked]
extern "x86-interrupt" fn timer_int(frame: InterruptStack) {
    unsafe {
        naked_asm!(
            "cli",
...
extern "C" fn timer_int_inner(
    context: crate::thread::ThreadContext,
) -> crate::thread::ThreadContext {
    /*let mut rsp1 = 0u64;
    unsafe { asm!(
        "mov {}, rsp",
        out(reg) rsp1,
...
extern "x86-interrupt" fn gpf_panic(frame: InterruptStack, error_code: u64) {
    let (mut a, mut c) = (0, 0);
    unsafe {
        asm!(
            "mov {},rax",
    ...
    panic!(
        "General Protection Fault, error: {:#018x} @ {:?}. RAX: {:#018x}, RCX: {:#018x}",
        error_code, frame, a, c
    )
}

extern "x86-interrupt" fn pf_panic(frame: InterruptStack, error_code: u64) {
    panic!("Page Fault, error: {:#018x} @ {:?}", error_code, frame)
}

extern "x86-interrupt" fn fpe_panic(frame: InterruptStack) {
    panic!("x87 Floating Point Exception @ {:?}", frame)
}

extern "x86-interrupt" fn ac_panic(frame: InterruptStack, error_code: u64) {
    panic!("Alignment Check, error: {:#018x} @ {:?}", error_code, frame)
}

extern "x86-interrupt" fn mce_panic(frame: InterruptStack) {
    panic!("Machine Check Exception @ {:?}", frame)
}

extern "x86-interrupt" fn simd_panic(frame: InterruptStack) {
    panic!("SIMD Floating Point Exception @ {:?}", frame)
}

extern "x86-interrupt" fn virt_panic(frame: InterruptStack) {
    panic!("Virtualization Exception @ {:?}", frame)
}

extern "x86-interrupt" fn cpe_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "Control Protection Exception, error: {:#018x} @ {:?}",
        error_code, frame
    )
}

extern "x86-interrupt" fn hypinj_panic(frame: InterruptStack) {
    panic!("Hypervisor Injection Exception @ {:?}", frame)
}

extern "x86-interrupt" fn vmm_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "VMM Communications Exception, error: {:#018x} @ {:?}",
        error_code, frame
    )
}

extern "x86-interrupt" fn sec_panic(frame: InterruptStack, error_code: u64) {
    panic!(
        "Security Exception, error: {:#018x} @ {:?}",
        error_code, frame
    )
}
